...
const Header = () => {
  ...
  return (
    <nav
      className={`flex fixed w-full z-10 h-28 px-10 justify-between items-center transition-transform duration-500 ease-in-out ${
        show ? "translate-y-0" : "-translate-y-28"
      } ${shadow ? "shadow-[0px_10px_30px_-10px_#020c1b]" : ""} xs:px-4 xs:h-20 bg-bgColor`}
    >
      <IconHexagonLetterH
        className="z-10"
        size={isMobile ? 45 : 60}
